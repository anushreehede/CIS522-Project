...
class Network(torch.nn.Module):
	...
	def forward(self, question, question_lengths, img_feat):
		...
		attended_img_feat = (ques_att_weights * img_feat).sum(1)#Apply attention to the image features [batch, k, 2048]
   

        
		#Pass both final attended img feat and obj feat through fc and sum
		final_att_fc_out = self.final_attended_fc(attended_img_feat) #[batch, 1280]
		...
		final_feats = final_ques_embed * final_att_fc_out
...
def train_model(train_dataloader, optimizer, loss, net, validation_loader, is_distributed, use_cuda, train_img_dict, val_img_dict, log='runs', epochs=5):
	...
	for epoch in range(epochs):        
		...
		torch.save(net.cpu().state_dict(), 'dl_advance_log/no_yolo/' + str(epoch) +'.pt')
		net = net.to(device)
	if not os.path.exists('dl_advance_log/no_yolo'):
		os.makedirs('dl_advance_log/no_yolo')
	np.save('dl_advance_log/no_yolo/train_loss.npy', training_losses, allow_pickle=True)
	np.save('dl_advance_log/no_yolo/train_acc.npy', training_accuracies, allow_pickle=True)
	np.save('dl_advance_log/no_yolo/val_loss.npy', validation_losses, allow_pickle=True)
	np.save('dl_advance_log/no_yolo/val_acc.npy', validation_accuracies, allow_pickle=True)
...
# 	vqa=VQA(train_set, val_set, annFile, quesFile)
    
	vqa=VQA(annFile, quesFile)
	# initialize VQA api for QA annotations
	annotations = filter_yes_no(vqa, dataSubType, bad_ids=np.load('bad_im_files_train.npy').tolist())
...
def get_dataset(loader, fields):
	...
	for i, (q, a, img_id) in enumerate(loader):
		...
		for q_i, a_i, img_id_i in zip(q, a, img_id):
			example_list.append(torchtext.data.Example.fromlist([q_i, a_i, img_id.detach().numpy()], fields))
...
def combine_datasets(train_loader, val_loader):
	...
	dill.dump(TEXT, open('train_val_vocab', 'wb'))
...
if __name__ == '__main__':
	...
	train_loader, train_img_dict = create_train_loader(data_dir, is_distributed, **kwargs)
	val_loader, val_img_dict  = create_val_loader(data_dir, **kwargs)
	...
	epochs = 20
	train_model(train_iter, optimizer_log, criterion_log, dl_net, val_iter, is_distributed, use_cuda,train_img_dict,  val_img_dict,  'dl_baseline', epochs=epochs)
    
	printTime("Finished training.........")
    
	torch.save(dl_net.cpu().state_dict(), 'dl_advance_log/no_yolo/' + str(epochs) +'.pt')
